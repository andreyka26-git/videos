...
const WIKIPEDIA_AGENT = 'User-Agent: CoolTool/0.0 (https://example.org/cool-tool/; cool-tool@example.org) generic-library/0.0';
...
async function main() {
    ...
    await Promise.all(rows.map(async row => {
        ...
        if (photoColumn && photoColumn.firstChild) {
            ...
            const source = imgNode.firstChild.getAttribute('src');
